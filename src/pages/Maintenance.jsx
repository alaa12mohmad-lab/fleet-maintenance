...
import { Wrench, Plus, Trash2, Edit2, ExternalLink } from 'lucide-react'
...
export default function Maintenance() {
  ...
  // map المعدات للحصول على الكود
  const itemMap = {}
  ;[...equipment.map(e => ({...e, _type:'equipment'})), ...vehicles.map(v => ({...v, _type:'vehicle'}))]
  ...
  const filtered = logs.filter(l => {
    ...
    return (
      ...
      l.maintenanceType?.toLowerCase().includes(q) ||
  ...
  const handleDelete = async () => {
    try {
      await deleteItem('maintenance_logs', deleteTarget.id)
  ...
  const exportExcel = () => {
    const data = filtered.map(l => ({
      'المعدة': l.equipmentName,
      'الكود': l.equipmentCode || '—',
      'نوع الصيانة': l.maintenanceType,
      'التاريخ': l.date,
      'قراءة العداد': l.meterReading,
      'التكلفة': l.cost,
      'الورشة': l.workshop,
      'الوصف': l.description,
      'ملاحظات': l.notes,
      'عدد المرفقات': l.attachments?.length || 0,
    }))
  ...
  return (
    <div className="space-y-5 animate-in">
      <div className="page-header">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            {logs.length} سجل &nbsp;·&nbsp; إجمالي التكاليف:
            <span className="text-emerald-400 font-semibold mr-1">{totalCost.toLocaleString()} ر.س</span>
          </p>
        </div>
      ...
      <SearchInput value={search} onChange={setSearch} placeholder="بحث بالمعدة أو الكود أو نوع الصيانة..." />
              ...
              <table className="w-full">
                <thead>
                  <tr className="table-header">
                    <th className="px-4 py-3 text-right">المعدة / الكود</th>
                    ...
                    <th className="px-4 py-3 text-right">مرفقات</th>
                ...
                <tbody>
                  {paginated.map(log => {
                    ...
                    const itemInfo = itemMap[log.equipmentId]
                    const code = log.equipmentCode || itemInfo?.code || itemInfo?.plateNumber || ''
                    return (
                      <tr key={log.id} className="table-row">
                        <td className="px-4 py-3">
                          <div className="font-semibold text-white text-sm">{log.equipmentName || '—'}</div>
                          {code && <div className="text-xs text-slate-500 font-mono">{code}</div>}
                        </td>
                        <td className="px-4 py-3 text-slate-300 text-sm">
                          <span className="bg-primary-900/40 text-primary-400 px-2 py-0.5 rounded text-xs">
                            {log.maintenanceType}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-sm">
                          {log.cost > 0
                            ? <span className="text-emerald-400 font-semibold">{Number(log.cost).toLocaleString()} ر.س</span>
                            : <span className="text-slate-500">—</span>}
                        </td>
                        <td className="px-4 py-3 text-slate-400 text-sm">{log.workshop || '—'}</td>
                        <td className="px-4 py-3 text-slate-500 text-xs max-w-[160px] truncate">{log.description || '—'}</td>
                        <td className="px-4 py-3">
